 ...
 * @version 1.0
 */
public class CSVFile2DB {
	private static Logger logger = LoggerFactory.getLogger(CSVFile2DB.class);
		...
		config.addConfig(new FileConfig().setSourcePath("D:\\workspace\\bbossesdemo\\csv-dbhandle\\excel\\csv")//指定目录
						.setFileFilter(new FileFilter() {
							...
							public boolean accept(FilterFileInfo fileInfo, FileConfig fileConfig) {
								...
								return fileInfo.getFileName().equals("table2.csv");
							}
						})//指定文件过滤器
...
//				.setUsePool(true)//是否使用连接池
				.setInsertSqlName("insertperson_year")//指定新增的sql语句名称，在配置文件中配置：sql-dbtran.xml
		...
		importBuilder.setDataRefactor(new DataRefactor() {
			public void refactor(Context context) throws Exception  {
				...
				//cert_no,  name , sex ,by_year  ,person_type

				context.addFieldValue("cert_no",values[0].toUpperCase());
				context.addFieldValue("name",values[1]);
				context.addFieldValue("sex",values[2]);
				context.addFieldValue("by_year",values[3]);
				context.addFieldValue("person_type",values[4]);
