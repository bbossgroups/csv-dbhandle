package org.frameworkset.elasticsearch.imp.handleyear;
/**
 ...
 * @version 1.0
 */
public class CSVFile1DB {
	private static Logger logger = LoggerFactory.getLogger(CSVFile1DB.class);
...
//				.setExcludeLines(new String[]{"\\[DEBUG\\]"}));//不采集debug日志
		...
		config.addConfig(new FileConfig().setSourcePath("D:\\workspace\\bbossesdemo\\csv-dbhandle\\excel\\csv")//指定目录
				.setFileFilter(new FileFilter() {
					@Override
					public boolean accept(FilterFileInfo fileInfo, FileConfig fileConfig) {
						//判断是否采集文件数据，返回true标识采集，false 不采集
						return fileInfo.getFileName().endsWith("table1.csv");
					}
				})//指定文件过滤器
		...
		importBuilder.addCallInterceptor(new CallInterceptor() {
			...
			public void preCall(TaskContext taskContext) {
				try {
					final Map<String,String> byyearByCertNo = new HashMap<>();//缓存用户编号到身份证
					SQLExecutor.queryWithDBNameByNullRowHandler(new NullRowHandler() {
						@Override
						public void handleRow(Record record) throws Exception {
							byyearByCertNo.put(record.getString("cert_no"),record.getString("by_year"));
						}
					},"test","select * from person_year");

					taskContext.addTaskData("byyearByCertNo",byyearByCertNo);
...
//				.setUsePool(true)//是否使用连接池
				.setInsertSqlName("insertnewmanrequests2")//指定新增的sql语句名称，在配置文件中配置：sql-dbtran.xml
		...
		importBuilder.setLastValueStorePath("newmanrequestfilelogdb2_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
		...
		importBuilder.setDataRefactor(new DataRefactor() {
			public void refactor(Context context) throws Exception  {
				...
				String person_no = values[1];
				if(person_no.startsWith("^"))
					person_no = person_no.substring(1);
				context.addFieldValue("person_no",person_no);

				context.addFieldValue("name",values[2]);
				String cert_no = values[5].toUpperCase();
				if(cert_no.startsWith("^"))
					cert_no = cert_no.substring(1);
				Map<String,String> byyearByCertNo = (Map<String,String>)context.getTaskContext().getTaskData("byyearByCertNo");
				String byYear = byyearByCertNo.get(cert_no);
				context.addFieldValue("by_year",byYear);
				context.addFieldValue("cert_type",values[4]);
				context.addFieldValue("cert_no",cert_no);
				context.addFieldValue("zhs_item",values[6]);
				context.addFieldValue("zhs_class",values[7]);
				context.addFieldValue("zhs_sub_class",values[8]);
				context.addFieldValue("rowNo",count.getCount());
				count.increament();
				if(values.length == 10) {
					context.addFieldValue("zhs_year", values[9]);
					context.addFieldValue("zhs_level", "");
				}
				else if(values.length == 11) {
					context.addFieldValue("zhs_year", values[9]);
					context.addFieldValue("zhs_level", values[10]);
				}
				else{
